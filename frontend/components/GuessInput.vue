...
const fuzzyPokemonNameMap = computed(() => {
  ...
  const result: LocalName[] = _.keys(api_data.pokemon_local_name_map).filter((local_name) => {
    return searcher.test(local_name.normalize("NFD"))
  }).map((local_name) => {
    const english_name = translatePokemonName(local_name, true)
    return {
      english_name: english_name,
      local_name: translatePokemonName(english_name, false), // to use proper-cased name
    }
  })
  return result
})
  ...
  if (
    ...
    const best_guess =
      sortedGuesses.length > 1
        ...
        : correct_guess
    statistics.last_best_guess = Object.assign({}, best_guess)
    statistics.last_correct_guess = Object.assign({}, correct_guess)
